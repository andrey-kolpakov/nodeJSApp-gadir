...
const handler = async (event, context) => {
    ...
    async function run(info) {
        ...
        const textMessageForGadir = `Вы только что поговорили с <b>${newObj.name}, теперь надо отправить сообщение в ватсап! <b>Номер:  ${newObj.phone} Перейди по ссылке, чтобы отправить сообщение: <a href="${textLink}">Ссылка</a>`
